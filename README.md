...
[cmetta.h](https://github.com/MesTTo/CMeTTa/blob/main/cmetta.h) defines every name and signature; [llms.txt](https://github.com/MesTTo/CMeTTa/blob/main/llms.txt) is the compact usage reference.
...
This version installs as `libcmetta.so.2`. Rebuild consumers and extensions together.
...
Each of the engine's closed value sets is a `(vocabulary ...)` row in `&metta`,
and each reaches C as an enum in [vocabularies.h](https://github.com/MesTTo/CMeTTa/blob/main/vocabularies.h), which
`cmetta.h` includes. `extensions/python/tools/vocabgen.py` generates the header
from the engine's own rows, beside the Python and Node tables, and the
`vocab-sync` lane fails when it drifts, so a misspelt member fails to compile
where a misspelt word would answer nothing.

```c
enum mt_answer_policy policy;
if ( mt_answer_policy_of("best-first", &policy) )
  printf("%s is member %d\n", mt_answer_policy_names[policy], (int)policy);
```

This prints `best-first is member 2`: a member's value is its position in the
...
| stack ceiling | SWI's stack bytes, which a runaway recursion or a very large collapse reaches; `.stack_bytes = 0` restores the ceiling the runtime booted under |

The runtime boots under the ceiling the host names in `mt_config.stack_limit`,
else the one `METTA_STACK_LIMIT` names, else `MT_STACK_LIMIT_DEFAULT`, the
8,000,000,000 bytes the Python seat declares. [settings.h](https://github.com/MesTTo/CMeTTa/blob/main/settings.h) carries
the default and the variable's name, generated by
`extensions/python/tools/boundsgen.py` from the Python seat's Setting
declarations, so the two seats boot under one number. `METTA_STACK_LIMIT` is
decimal digits and positive, and anything else refuses the boot with
`MT_MISUSE` in the Python seat's words:

```c
setenv("METTA_STACK_LIMIT", "eight gigabytes", 1);
if ( !mt_open(NULL) )
    fprintf(stderr, "%s\n", mt_errmsg());
    /* METTA_STACK_LIMIT must be a positive integer, got 'eight gigabytes' */
```
